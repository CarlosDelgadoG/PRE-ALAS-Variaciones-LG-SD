...
```{r}
tabla_IC <- readRDS("MODELOS/mlm_tabla_ic.RDS")
tabla_mlm<-tibble(coef=c(var_coefs,'Tiempo'),
              valor= summary(mlm_multi)$coefficients[,1],
              L=tabla_IC[-c(1,2),1],
              U=tabla_IC[-c(1,2),2])

tabla_mlm$tipo_var <- factor(with(tabla_mlm,
     case_when(
       (L>0&U>0) ~2,
       (L<0&U<0)~3,
       TRUE~1)),
...
```{r}
ggplot(filter(tabla_mlm,!(coef=='Intercepto')),aes(y=coef,x=valor,color=factor(tipo_var)))+
  geom_point()+
  geom_errorbar(aes(xmin=L, xmax=U), width=.2)+
  scale_color_viridis(discrete = TRUE,option = "D")+
  geom_vline(xintercept = 0,linetype="dashed")+
